...
import numpy as np
import pandas as pd
from tabularepimdl.arrayops import grouped_count as grouped_count_array
from tabularepimdl.matrixops import matrix_grouped_count
...
def setup_data(N, G):
    ...
    group_matrix = np.zeros((G, N), dtype=np.float32)
    group_matrix[group_ids, np.arange(N)] = 1.0

    return values, group_ids, group_matrix

def run_benchmark_case(dispatch_type, values, group_ids, group_matrix, T):
    if dispatch_type == "array":
        result = np.zeros((T, group_ids.max() + 1), dtype=np.float32)
        for t in range(T):
            result[t] = grouped_count_array(values, group_ids, result.shape[1])
        return result, result.nbytes

    elif dispatch_type == "matrix":
        result = np.zeros((T, group_matrix.shape[0]), dtype=np.float32)
        for t in range(T):
            result[t] = matrix_grouped_count(group_matrix)
        return result, result.nbytes
...
def run_benchmarks():
    ...
    results_file, _ = generate_results_file("array_vs_matrix_grouped_count")

    for N in Ns:
        for G in Gs:
            values, group_ids, group_matrix = setup_data(N, G)
