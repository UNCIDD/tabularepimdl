...
def multistrain_infectiousprocess(betas, cross_protect):
    return MultiStrainInfectiousProcess_Vec_Encode(
        betas=betas, columns=["Strain1", "Strain2"], columns_all_categories=["S", "I", "R"],
        cross_protect=cross_protect, infstate_compartments=infstate_compartments,
    )
...
def test_initialization_mismatched_betas_and_columns_raises(cross_protect):
    with pytest.raises(ValueError):
        MultiStrainInfectiousProcess_Vec_Encode(
            betas=np.array([0.1, 0.05, 0.2]), columns=["Strain1", "Strain2"], columns_all_categories=["S", "I", "R"],
            cross_protect=cross_protect, infstate_compartments=infstate_compartments,
        )
...
def test_initialization_non_square_cross_protect_raises(betas):
    with pytest.raises(ValueError):
        MultiStrainInfectiousProcess_Vec_Encode(
            betas=betas, columns=["Strain1", "Strain2"], columns_all_categories=["S", "I", "R"],
            cross_protect=np.array([[1.0, 0.5, 0.2], [0.5, 1.0, 0.3]]), infstate_compartments=infstate_compartments,
        )
