...
nc_deaths = [
     ...
     StateBasedDeathProcess(columns=["AgeCat"], states=["10 to 19"], rate=.1/1000, stochastic=False),
...
def aging_determ_vec1_5_model():
    ...
    determ_epi_mdl_vec1_5 = EpiModel_Vec_Encode_1_5(init_state=add_infect,
...
#Parameterized test
# -------------------
@pytest.mark.parametrize("model_label, model_fixture_name, step_fn",
[
    ("pandas", "aging_determ_pandas_model", run_pandas_step),
    ("vec1_5", "aging_determ_vec1_5_model", run_vec_step)
])
def test_model_performance_and_output(request, model_label, model_fixture_name, step_fn, infect, benchmark_results):
    ...
    if model_label == "pandas":
        ...
        #print('pandas epi\n', model.full_epi)
        pandas_sorted = model.full_epi.sort_values(by=['T', 'InfState', 'AgeCat', 'N'], ascending=[True, True, True, True]) #sort values before comparing pandas and array result to make sure the order of N lines up
        ...
        vec_sorted = model.full_epi().sort_values(by=['T', 'InfState', 'AgeCat', 'N'], ascending=[True, True, True, True]) #sort values before comparing pandas and array result to make sure the order of N lines up
