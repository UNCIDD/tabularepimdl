...
class WAIFWTransmissionMpox(Rule, BaseModel):
    """!
    ...
    def validate_waifw_matrix(cls, matrix_parameters, field: ValidationInfo):
        ...
        return matrix_parameters #NOT transpose the input matrix for Mpox application
    ...
    def get_deltas(self, current_state: pd.DataFrame, dt: int | float = 1.0, stochastic: bool | None = None) -> pd.DataFrame:
        ...
        return rc.reset_index(drop=True)
    ...
    def to_yaml(self) -> dict:
        ...
        rc = {
            'tabularepimdl.WAIFWTransmission' : {
                'waifw_matrix' : self.waifw_matrix, #not transpose waifw matrix for Mpox application
