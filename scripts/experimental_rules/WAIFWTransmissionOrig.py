...
class WAIFWTransmissionOrig(Rule):
    """!
    ...
    def get_deltas(self, current_state, dt = 1.0, stochastic = None):
        ...
        #print('prI codes are', prI_per_group, '\n') #debug

        ##do infectious process, getting the number of individuals who get infected from susceptible status
        if not stochastic:
            deltas = deltas.assign(N=-deltas['N']*prI_per_group)
        else:
            deltas = deltas.assign(N=-np.random.binomial(deltas['N'], prI_per_group))
