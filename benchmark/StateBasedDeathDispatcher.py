...
class StateBasedDeathProcessDispatcher(BaseModel):
    ...
    def model_post_init(self, _):  # initialize dispatcher based on data structures
        if self.structure == "Pandas":
            self._dispatcher = StateBasedDeathProcess(columns=self.columns, states=self.target_states, rate=self.rate, stochastic=self.stochastic)
        elif self.structure == "Numpy_Encode":
            self._dispatcher = StateBasedDeathProcess_Vec_Encode(
                column=self.column, column_states=self.column_states, target_states=self.target_states, rate=self.rate, stochastic=self.stochastic, infstate_compartments=self.infstate_compartments
            )
        else:
    ...
    def get_deltas(
        ...
        if self.structure == "Numpy_Encode":
