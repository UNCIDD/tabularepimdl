...
class BirthProcessDispatcher(BaseModel):
    ...
    def model_post_init(self, _):  # initialize dispatcher based on data structures
        if self.structure == "Pandas":
            self._dispatcher = BirthProcess(rate=self.rate, start_state_sig=self.start_state_sig, stochastic=self.stochastic)
        elif self.structure == "Numpy_Encode":
            self._dispatcher = BirthProcess_Vec_Encode(rate=self.rate, column_to_sort=self.column_to_sort, stochastic=self.stochastic, infstate_compartments=self.infstate_compartments)
        else:
