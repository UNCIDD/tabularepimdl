...
class EpiModel(BaseModel):
    ...
    def _setup_internal_arrays(self):
        ...
        # === Encode compartments
        compartments = sorted(df[self.compartment_col].unique())
        self.comp_map = {label: i for i, label in enumerate(compartments)}
        self.inv_comp_map = {v: k for k, v in self.comp_map.items()}
        df[self.compartment_col] = df[self.compartment_col].map(self.comp_map).astype(np.int32)

        # === Initialize columns and index mapping
    ...
    def do_timestep(self):
        ...
        for ruleset in self.rules:
            for rule in ruleset:
                rule.apply(self.cur_state_arr, self.col_idx, self.dt, out=delta_arr)

        self.cur_state_arr += delta_arr
        self.cur_state_arr[:, self.col_idx["T"]] = self.t_current + self.dt
