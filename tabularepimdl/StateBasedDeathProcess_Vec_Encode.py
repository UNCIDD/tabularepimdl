...
class StateBasedDeathProcess_Vec_Encode(Rule, BaseModel):
    ...
    Attributes:
        ...
        states: the states of single column.
    ...
    states = ['I', 'S'] #values from single column
    ...
    states: list[str] = Field(description = "the state of single column.")
    ...
    @field_validator("states", mode="before") #validate list type
    @classmethod
    ...
    def model_post_init(self, _):
        ...
        states_sorted = sorted(self.states)
        colstate_to_int = {s: i for i, s in enumerate(states_sorted)} #for the single column mapping
        self._states_code = [colstate_to_int[state] for state in states_sorted] #encoded column states
