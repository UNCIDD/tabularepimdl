...
def normalize_fields(name, fields):
    if isinstance(fields, str):
        raise TypeError(f"{name} of domain_membership_validator must be a tuple/list of field names, not a string. Did you mean ({fields!r},)?")
...
def domain_membership_validator(attribute_fields: Iterable[str], domain_fields: Iterable[str]):
    """
    ...
    def validator(self):
        ...
        for field in attribute_fields_:
            ...
            if invalid_values:
                raise ValueError(f"{field} contains invalid value(s): {invalid_values}. Allowed values are: {sorted(domain)}.")
