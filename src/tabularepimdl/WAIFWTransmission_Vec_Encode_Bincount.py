...
class WAIFWTransmission_Vec_Encode_Bincount(Rule, BaseModel):
    ...
    group_col_all_categories: UniqueNonEmptyStrIntUniformList = Field(description="all the categories the group column should have.")
    ...
    _check_domain_membership = domain_membership_validator(attribute_fields=("s_st", "i_st", "inf_to"), domain_fields=("infstate_compartments",))
    ...
    def get_deltas(self, current_state: np.ndarray, col_idx_map: dict[str, int], result_buffer: np.ndarray, dt: float = 1.0, stochastic: bool | None = None) -> np.ndarray:
        ...
        if len(self.group_col_all_categories) != len(self.waifw_matrix):
            raise ValueError(
                f"Mismatch between the number of unique categories of input data and WAIFW matrix size. "
    ...
    def to_dict(self) -> dict:
        ...
        """
        rc = {
            "tabularepimdl.WAIFWTransmission_Vec_Encode_Bincount": {
